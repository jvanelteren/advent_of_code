...
class State(dict):
    ...
    def set_available(self):
        avail = []
        res = ''.join([''.join(sorted(v)) if v else '' for k,v in self.items() if k in [0,1,2,3]])
    ...
    def recode(self):
        res = ''.join([''.join(v) for k,v in self.items() if k in [0,1,2,3] and v])
        ...
        for k in self:
            if k in [0,1,2,3]:
                super().__setitem__(k,list(''.join(self[k]).translate(code)))
                self.set_available()
        ...
        if (k != 'elev' and v):
            ...
            for i in v:
                if not i.lower() in to_remove and not i.isdigit():
                    newstring+=i
            super().__setitem__(k,list(newstring))
        else:
            super().__setitem__(k,v)

state = {0:list('4'), 1: list('2A'),2:list('0a'),3:list('0'),'elev':['0']}
