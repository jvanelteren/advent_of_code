...
#%%
def stringlist_to_int(inp): # returns items converted to int if possible. also works for tuples
    if isinstance(inp[0],list):
        return [stringlist_to_int(l) for l in inp]
    if isinstance(inp[0],tuple):
        return tuple(stringlist_to_int(l) for l in inp)
